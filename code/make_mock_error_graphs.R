...
before_precluster <- error_data[["mock_precluster_error"]] %>% 
  ...
  ggtitle("A") + 
  annotate("text", label = paste("Before Pre-Cluster Step"), x = 2.5, y = 0.031, size = 2.5) + 
...
before_chimera_vsearch <- error_data[["mock_chimera_error"]] %>% 
  ...
  labs(x = "Amplification Cycles", y = "Median Average Error Rate Per Base") + 
  ggtitle("B") + 
  annotate("text", label = paste("Before Chimera VSEARCH Step"), x = 2.5, y = 0.031, size = 2.5) + 
  theme(plot.title = element_text(face="bold", hjust = -0.07, size = 20), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        axis.text.y = element_text(size = 10), 
...
full_pipeline <- error_data[["mock_error"]] %>% 
  ...
  labs(x = "Amplification Cycles", y = "Median Average Error Rate Per Base") + 
  ggtitle("C") + 
  annotate("text", label = paste("Full Pipeline"), x = 2.5, y = 0.031, size = 2.5) + 
  theme(plot.title = element_text(face="bold", hjust = -0.07, size = 20), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        axis.text.y = element_text(size = 10), 
        legend.position = c(0.5, 0.63), 
        legend.title = element_blank(), 
        legend.key = element_blank(), 
        legend.background = element_rect(color = "black"))
