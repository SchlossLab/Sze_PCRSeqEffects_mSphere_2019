...
Differences by HiFi Taq in the median average per base error varied without a clear pattern across sub-sampling depth [Table S5]. Generally, the highest values were for the Kappa HiFI DNA polymerase [Figure 3A]. Sub-sampling depth seems to have little effect on this rate with both 5000 and 10000 sub-sampled sequences showing similar results [Figure 3B-C]. There were small differences between the various HiFi Taq at lower cycle number but larger differences at higher cycle number with Platinum having large differences between all other HiFi DNA polymerase [Figure 3B-C and Table S6]. 
...
Although we did not observe strong differences, based on cycle number, using the Bray-Curtis index the data suggests that there may be differences between 15 cycles and higher cycle numbers, such as 30, that are commonly used. What is most interesting is that there was no difference within individuals between corresponding 5 cycle increments (e.g. 15 to 20, 20 to 25, etc.). This finding, in conjunction with the PERMANOVA results, would suggest that differences based on Bray-Curtis incrementaly build up so that within 5 cycle increments the differences are not that large but when comparing larger cycle differences, as with the PERMANOVA, these differences start to become detectable. Although our data suggests that it is possible to overcome these types of cycle dependent differences with increased sub-sampling depth, for some DNA polymerase increased sub-sampling depth increases chimera prevalence and per base error rate. Conversely, it is possible that many of the increased number of OTUs, generated as cycle number increases, are not highly abundant and the Bray-Curtis index is able to successfully downweight these respective OTUs [@bc_index_Minchin1987].   
