...
inline_hook <- function(x){
	...
	if(is.numeric(x)){
		if(abs(x - round(x)) < .Machine$double.eps^0.5){
			...
			paste(format(x,big.mark=',', digits=2, nsmall=3, scientific=FALSE))
		}
	} else {
    	paste(x)      
	}
...
```{r errors}
error <- read_tsv("../data/process/error_chimera_rates.tsv") %>%
	filter(rounds == 30) %>%
	select(polymerase, contig_error, pc_error) %>%
	mutate(contig_error = 100 * contig_error,
				pc_error = 100 * pc_error)
```

***Sequencing errors vary by the number of cycles and the DNA polymerase used in PCR.*** The presence of sequence errors can confound the ability to accurately classify 16S rRNA gene sequences and group sequences into operational taxonomic units (OTUs). More importantly, sequencing errors themselves can alter the representation of the community. Therefore, it is important to minimize the number of sequencing errors. Using a widely-used approach that generates the lowest reported error rate, we quantified the error rate by sequencing the V4 region of the 16S rRNA genes from an 8 member mock community. We also removed any contigs that were at least three bases more similar to a chimera of two references than to a single reference sequence [@Kozich2013; @Schloss2011; @Quince2011]. Regardless of the polymerase, the error rate increased with the number of rounds of amplification (Figure 1). Using 30 rounds of PCR is a common approach across diverse types of samples. Among the data generated using 30 rounds of PCR the Accuprime polymerase had the highest error rate (i.e. `r filter(error, polymerase=="ACC") %>% select(contig_error)`%) followed by the Platinum (i.e. `r filter(error, polymerase=="PL") %>% select(contig_error)`%), Phusion (i.e. `r filter(error, polymerase=="PHU") %>% select(contig_error)`%), Kappa (i.e. `r filter(error, polymerase=="K") %>% select(contig_error)`%), and Q5 (i.e. `r filter(error, polymerase=="Q5") %>% select(contig_error)`%) polymerases (Figure 1). When we applied the pre.clustering denoising step, which merges the counts of reads within 2 nt of a more abundant sequence, the error rates dropped considerably such that the Platinum polymerase had the highest error rate (i.e. `r filter(error, polymerase=="PL") %>% select(pc_error)`%) followed by the Accuprime (i.e. `r filter(error, polymerase=="ACC") %>% select(pc_error)`%), Q5 (i.e. `r filter(error, polymerase=="Q5") %>% select(pc_error)`%), Phusion (i.e. `r filter(error, polymerase=="PHU") %>% select(pc_error)`%), and Kappa (i.e. `r filter(error, polymerase=="K") %>% select(pc_error)`%) polymerases (Figure 1). Although specific recommendations are difficult to make because the impact of the actual community structure and concentration of the initial DNA template are likely to have an impact on the results, it is clear that using as few PCR cycles as necessary and a polymerase with the lowest possible error rate is a good guide to minimizing the impact of polymerase on the error rate.


```{r chimeras}
chimera <- read_tsv("../data/process/error_chimera_rates.tsv") %>%
	filter(rounds == 30) %>%
	select(polymerase, pc_frac_chimera) %>%
	mutate(pc_frac_chimera = 100 * pc_frac_chimera)

min_specificity <- read_tsv("../data/process/error_chimera_rates.tsv") %>%
	filter(rounds == 30) %>%
	select(vsearch_sens, vsearch_spec) %>%
	summarize(min_spec = min(100 * vsearch_spec))

sensitivity <- read_tsv("../data/process/error_chimera_rates.tsv") %>%
	filter(rounds == 30) %>%
	select(vsearch_sens, vsearch_spec) %>%
	summarize(min = min(100*vsearch_sens), max=max(100*vsearch_sens))
...
stool_drift <- read_tsv("../data/process/stool_beta_diversity.tsv") %>%
	...
	ungroup() %>%
	filter(n == 6) %>%
	summarize(min = min(ave_dist), max = max(ave_dist))
```

***There is little evidence of a relationship between polymerase or number of rounds of amplification on PCR drift.*** There have been concerns that the same template DNA subjected to the same PCR conditions could result in different representations of communities because of random drift. To test this, we determined the average Bray-Curtis distance between replicate reactions using the same polymerase and number of rounds of amplification (Figure 6). Using the mock community data there were no obvious trends. The average Bray-Curtis distance within a set of conditions varied by `r mock_drift["min"]` to `r mock_drift["max"]` units. Although we did not obtain replicates of each of the stool samples, the intra-sample variation for each set of conditions was consistent and varied between `r stool_drift["min"]` and `r stool_drift["max"]` units. These data suggest that amplicon sequencing is robust to random variation in amplification and that any differences are likely to be smaller than what is considered biologically relevant.
