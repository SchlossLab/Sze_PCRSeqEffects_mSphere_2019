...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Results have an impact on downstream diversity measurements due to effects on total OTUs}


\CommentTok{#Need to cover that different types of polymerase may affect outcome}
\CommentTok{#- Accuprime HiFi from recombinant A-Family Taq DNA polymerase, Pyrococcus spp GB-D polymerase (proof-reading), platinum Taq Antibody (inhibits room temp activity)}

\CommentTok{#- Kappa HiFi from B-family DNA polymerase (proof-reading baked in), propietary antibody (inhibit room temp activity)}

\CommentTok{#- Q5 HiFi from B-family thermostable DNA polymerase with 3´→ 5´ exonuclease activity, fused to a processivity-enhancing Sso7d domain to support robust DNA amplification}

\CommentTok{#-Phusion HiFi B-family DNA polymerase a novel Pyrococcus-like enzyme fused with a processivity-enhancing domain, generate blunt-ended products.}

\CommentTok{#- Platinum HiFi recombinant A-Family Taq DNA polymerase, Pyrococcus species GB-D polymeras, Platinum Taq Antibody}
\end{Highlighting}
\end{Shaded}
