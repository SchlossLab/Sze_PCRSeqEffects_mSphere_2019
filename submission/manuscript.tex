...
Number:}} Using PERMANOVA to test whether there were any differences
within HiFi DNA polymerase groups based on cycle number, only Phusion
had cycle dependent differences at 1000 and 5000 sub-sampling depth
(P-value = 0.03 and 0.01, respectively). Interestingly, Phusion was one
of only two DNA polymerase that managed to have samples for the 1000
sub-sampling depth at 15 cycles. Next, we assessed whether there were
any major differences between 5 cycle increments within each sample. We
found that there was no detectable difference in Bray-Curits index when
comparing to the previous 5 cycle increment (P-value \textgreater{}
0.05). However, Phusion at 1000 sub-sampling depth had a P-value = 0.02
before multiple comparison correction. It should be noted that at higher
...
{[}Figure 3A{]}. Sub-sampling depth seems to have little effect on this
rate with both 5000 and 10000 sub-sampled sequences showing similar
results {[}Figure 3B-C{]}. There were small differences between the
...
HiFi DNA polymerase {[}Figure 3B-C and Table S6{]}.
...
the HiFi DNA Polymerase {[}Figure 4{]}.
...
Platinum and Phusion HiFi DNA Polymerase {[}Figure 5{]}. In general, the
...
differences between 15 cycles and higher cycle numbers, such as 30, that
are commonly used. What is most interesting is that there was no
difference within individuals between corresponding 5 cycle increments
(e.g.~15 to 20, 20 to 25, etc.). This finding, in conjunction with the
PERMANOVA results, would suggest that differences based on Bray-Curtis
incrementaly build up so that within 5 cycle increments the differences
are not that large but when comparing larger cycle differences, as with
the PERMANOVA, these differences start to become detectable. Although
our data suggests that it is possible to overcome these types of cycle
dependent differences with increased sub-sampling depth, for some DNA
polymerase increased sub-sampling depth increases chimera prevalence and
per base error rate. Conversely, it is possible that many of the
increased number of OTUs, generated as cycle number increases, are not
highly abundant and the Bray-Curtis index is able to successfully
downweight these respective OTUs (Minchin, 1987).
